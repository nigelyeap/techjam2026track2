...
def encode_ext(splits, feature_set=('rate',), halflives=HALFLIVES, alpha=ALPHA, n_buckets=10):
    ...
    train-fit quantile bucketing (`n_buckets` buckets, default 10 matching
    iter9/iter16). `alpha` is the Laplace-smoothing constant used in BOTH the
    flat `rate` formula and the `decay_rate_H` formula (iter21: made
    sweepable — iter16 hardcoded this at module-level ALPHA=1.0).
    Returns (enc, field_dims_sum) with enc[name] = (X, y, users)."""
    tr = splits['train']
    dur_edges = _bucket_edges([x[5] for x in tr], n=n_buckets)
    ...
    for name in feature_set:
        ...
        if kind == 'activity':
            extra_edges[name] = _bucket_edges([x[7] for x in tr], n=n_buckets)
        elif kind == 'tab':
            extra_edges[name] = _bucket_edges([x[8] for x in tr], n=n_buckets)
        elif kind == 'rate':
            extra_edges[name] = _bucket_edges([(x[9] + alpha) / (x[10] + 2 * alpha) for x in tr], n=n_buckets)
