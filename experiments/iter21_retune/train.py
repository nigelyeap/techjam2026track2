"""iter16: FM + activity-weighted BPR pairwise loss (identical loss/sampling
mechanism to iter9/iter3), fed the EXTENDED feature encoding from data_ext.py
which supports both iter9's flat causal features (activity/tab/rate) AND new
recency-decayed (exponential half-life) versions (decay_rate_H, decay_act_H).

Loss, optimizer, sampling, hyperparams are all a line-for-line copy of
experiments/iter9_history_dense/train.py (which itself copied iter3).
Reuses baseline.FM unmodified and evaluate.py's evaluate() unmodified.
"""
...
from data_ext import load_ext, encode_ext, BASE_FIELDS, HALFLIVES, ALPHA  # noqa: E402
...
def run_bpr_ext(data_dir, feature_set=('rate',), halflives=HALFLIVES, k=16, lr=0.001, epochs=40,
    ...
    alpha: Laplace-smoothing constant for rate/decay_rate features (iter21: sweepable).
    n_buckets: quantile bucket count for all bucketed continuous features (iter21: sweepable)."""
    splits = splits_cache if splits_cache is not None else load_ext(data_dir, halflives=halflives)
    enc, dim = encode_ext(splits, feature_set=feature_set, halflives=halflives, alpha=alpha, n_buckets=n_buckets)
...
if __name__ == '__main__':
    ...
    res = run_bpr_ext(a.data_dir, feature_set=feature_set, k=a.k, lr=a.lr, epochs=a.epochs,
                       ...
                       steps_mult=a.steps_mult, alpha=a.alpha, n_buckets=a.n_buckets)
    print(f"\n=== iter16 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
