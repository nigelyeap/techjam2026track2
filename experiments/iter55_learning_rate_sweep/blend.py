"""iter55 (part 2): blend the linear_tree=True + learning_rate=0.10 GBM
with the FM ensemble, exactly as iter51's blend.py did for lr=0.05.

iter55's standalone finding: learning_rate=0.10 beats iter51's lr=0.05
baseline by +0.00085 valid (5-seed mean), 5/5 seeds improving, tight
seed variance. This checks whether the gain also survives blending with
the FM ensemble the same way iter51's linear_tree finding did.
"""
...
_gbm51 = _load_module(os.path.join(_THIS_DIR, '..', 'iter51_linear_tree', 'train.py'), 'iter55_blend_iter51_train')
...
def main():
    print("=== GBM (linear_tree=True, learning_rate=0.10, iter55) ===", flush=True)
    ...
    print(f"\n  [compare] iter51 blend (lr=0.05, alpha=0.08): valid=0.67297 test=0.65643")
    print(f"  [compare] iter55 blend (lr=0.10, alpha={alpha:.2f}): valid={best['valid']:.5f} test={te_m['primary']:.5f}")
    ...
    with open(os.path.join(_THIS_MOD_DIR, 'blend_results.json'), 'w') as f:
        json.dump({
            ...
            'iter51_blend_for_comparison': {'alpha': 0.08, 'valid': 0.67297, 'test': 0.65643},
        }, f, indent=2)
