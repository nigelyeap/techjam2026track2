...
Motivation: the FM baseline trains on pointwise logloss (per-row "is this a
long_view?") but is graded on within-user ranking metrics (GAUC / nDCG@5).
BPR closes that mismatch: for each training step, sample (user, positive
row, negative row) triples from the SAME user and push score(pos) above
score(neg). Loss = -log(sigmoid(score_pos - score_neg)).
