"""iter72: decayed per-(user, duration-bucket) RATE as a GBM-native feature,
added on top of iter63's rate_only feature set. Same mechanism as
decay_tab_rate_3, applied to a bucketed duration_ms key instead of tab --
see this dir's data_ext.py docstring for motivation (duration bucket is a
coarse, cross-cutting CONTENT property like tab, unlike author/tag which
iter69/71 showed are redundant with existing finer-grained categoricals).
"""
...
_de = _load_module(os.path.join(_THIS_DIR, 'data_ext.py'), 'iter72_data_ext')
ALPHA = 0.5
...
def _row_to_dict(x):
    ...
    for h in _de.DUR_HALFLIVES:
        dcol_pos = _de._dur_halflife_col(h, _de.HALFLIVES, _de.TAB_HALFLIVES, _de.DUR_HALFLIVES)
        dcol_tot = _de._dur_halflife_total_col(h, _de.HALFLIVES, _de.TAB_HALFLIVES, _de.DUR_HALFLIVES)
        d_pos = x[dcol_pos]
        d_tot = x[dcol_tot]
        d[f'decay_dur_rate_{h}'] = float((d_pos + ALPHA) / (d_tot + 2 * ALPHA))
    return d


VARIANT_NUM_COLS = {
    'baseline': BASE_NUM_COLS,
    'dur_rate_h3': BASE_NUM_COLS + ['decay_dur_rate_3'],
    'dur_rate_h7': BASE_NUM_COLS + ['decay_dur_rate_7'],
    'dur_rate_both': BASE_NUM_COLS + ['decay_dur_rate_3', 'decay_dur_rate_7'],
}
...
if __name__ == '__main__':
    ...
    for tag in ('dur_rate_h3', 'dur_rate_h7', 'dur_rate_both'):
    ...
    for tag in ['baseline', 'dur_rate_h3', 'dur_rate_h7', 'dur_rate_both']:
        v, tt = results[tag]
