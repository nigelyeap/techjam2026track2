"""iter33: iter28's exact setup (DeepFM stacked on iter24's refined feature
set) PLUS iter30's `init_scale_mult` variance-reduction lever threaded
through the constructor. This file is a copy of
iter28_deepfm_refined_features/train.py with `init_scale_mult` added as a
passthrough kwarg to `run_deepfm_bpr` and the `DeepFM(...)` constructor call
(and a CLI flag) -- no other training-loop logic changed. `init_scale_mult=1.0`
(the default) is an exact no-op and must bit-reproduce iter28's own numbers;
this is verified as this iteration's harness-fidelity check (see driver.py
Phase 0) before applying `init_scale_mult=0.5` (iter30's recommended lever).

Original iter28 docstring, unmodified below:

DeepFM-style model (baseline.FM's linear+pairwise terms unchanged,
...
def run_deepfm_bpr(data_dir, feature_set=DEFAULT_FEATURES, halflives=HALFLIVES,
    ...
    if verbose:
        ...
        print(f"  fields={fields} dim={dim} num_fields={num_fields} hidden={hidden} use_deep={use_deep} "
              f"init_scale_mult={init_scale_mult}")
    ...
    m = DeepFM(dim, num_fields=num_fields, k=k, hidden=hidden, lr=lr, mlp_lr=mlp_lr,
               l2_mlp=l2_mlp, use_deep=use_deep, seed=seed, init_scale_mult=init_scale_mult)
...
if __name__ == '__main__':
    ...
    print(f"loading {a.data_dir} ... extra features={feature_set} hidden={hidden} use_deep={not a.no_deep} "
          f"init_scale_mult={a.init_scale_mult}")
    res = run_deepfm_bpr(a.data_dir, feature_set=feature_set, k=a.k, hidden=hidden,
                          ...
                          init_scale_mult=a.init_scale_mult,
                          epochs=a.epochs, bs=a.bs, patience=a.patience, seed=a.seed,
                          verbose=not a.quiet, K=a.K)
    print(f"\n=== iter33 deepfm+refined_features+stabilized hidden={hidden} use_deep={not a.no_deep} "
          f"init_scale_mult={a.init_scale_mult} (seed={a.seed}) ===")
