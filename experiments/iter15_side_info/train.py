"""iter15: FM + activity-weighted BPR pairwise loss (identical loss/sampling
mechanism to iter9/iter3), fed the EXTENDED feature encoding from
data_ext.py: base 5 fields + iter9's causal history features
({activity,tab,rate}) + optionally static side-info fields from
user_features_pure.csv and/or video_features_statistic_pure.csv (never used
elsewhere in this run). Isolates exactly one variable vs iter9: whether
side-info fields are added on top of iter9's already-best feature set.

Reuses baseline.FM and evaluate.py's evaluate() unmodified. bpr_step,
sample_pairs, build_pos_neg_index are a line-for-line copy of
iter9_history_dense/train.py (which itself copied iter3 verbatim).
"""
...
def run_bpr_ext(data_dir, feature_set=('activity', 'tab', 'rate'), k=16, lr=0.001, epochs=40,
                 bs=8192, patience=4, seed=0, verbose=True, steps_mult=1, _cached_splits=None):
    splits = _cached_splits if _cached_splits is not None else load_ext(data_dir)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--features', default='activity,tab,rate',
                    help="comma-separated subset/order of "
                         "{activity,tab,rate,u_active_degree,u_live_streamer,"
                         "u_video_author,u_follow_range,u_fans_range,u_register_range,"
    ...
    print(f"\n=== iter15 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
