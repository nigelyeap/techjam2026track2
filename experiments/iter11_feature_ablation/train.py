"""iter11: copy of iter9_history_dense/train.py (FM + activity-weighted BPR
pairwise loss + extended causal-history feature encoding from data_ext.py),
adapted for a feature-set / Laplace-smoothing-constant ablation. Two changes
vs iter9's train.py:
  1. run_bpr_ext accepts an `alpha` kwarg, plumbed through to encode_ext, so
     the smoothing constant used in the 'rate' feature's formula
     (prior_pos+alpha)/(prior_total+2*alpha) can be swept instead of being
     hardcoded (iter9 always used alpha=1.0).
  2. run_bpr_ext accepts an optional pre-loaded `splits` dict so a sweep
     driver can call load_ext() once and reuse it across many configs/seeds.
Everything else (FM, BPR loss, activity-weighted sampling, hyperparams) is
unchanged from iter9's train.py, which is itself a line-for-line copy of
experiments/iter3_bpr_weighted/train.py. Reuses baseline.FM unmodified and
evaluate.py's evaluate() unmodified. Does NOT modify iter9's files.
"""
import argparse, os, sys, time, json
import numpy as np
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..'))
from evaluate import evaluate           # noqa: E402  official eval, unmodified
from baseline import FM, sigmoid        # noqa: E402  same FM class as iter3

from data_ext import load_ext, encode_ext, BASE_FIELDS, ALPHA  # noqa: E402


def build_pos_neg_index(y, users):
    """Identical to iter3_bpr_weighted/train.py — copied verbatim."""
...
def run_bpr_ext(data_dir, feature_set=('activity',), k=16, lr=0.001, epochs=40, bs=8192,
                 patience=4, seed=0, verbose=True, steps_mult=1, alpha=ALPHA, splits=None):
    """iter11: identical to iter9's run_bpr_ext, plus an `alpha` param (Laplace
    smoothing constant for the 'rate' feature, plumbed through to encode_ext)
    and an optional pre-loaded `splits` dict (from load_ext) so a sweep driver
    can load the raw data + causal features once and reuse across configs."""
    if splits is None:
        splits = load_ext(data_dir)
    enc, dim = encode_ext(splits, feature_set=feature_set, alpha=alpha)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--features', default='activity',
                    help="comma-separated subset/order of {activity,tab,rate}, "
                         "e.g. 'activity' or 'activity,tab' or 'activity,tab,rate'")
    ...
    ap.add_argument('--alpha', type=float, default=ALPHA,
                    help="Laplace smoothing constant for the 'rate' feature (iter11 addition)")
    ap.add_argument('--quiet', action='store_true')
    a = ap.parse_args()
    feature_set = tuple(f for f in a.features.split(',') if f)
    print(f"loading {a.data_dir} ... extra features={feature_set} alpha={a.alpha}")
    res = run_bpr_ext(a.data_dir, feature_set=feature_set, k=a.k, lr=a.lr, epochs=a.epochs,
                       ...
                       steps_mult=a.steps_mult, alpha=a.alpha)
    print(f"\n=== iter11 fm+bpr_weighted+{'+'.join(feature_set)} alpha={a.alpha} (seed={a.seed}) ===")
