"""iter63 (part 3): blend the rate_only GBM (decay_tab_rate_3 replacing
decay_tab_3) with the FM ensemble, exactly as iter55/iter61's blend.py did.

Standalone finding: rate_only beats baseline (iter55's exact feature set)
by +0.00107 valid (5-seed mean), 5/5 seeds, test also +0.00098. This
checks whether the gain survives blending with the unchanged FM ensemble.
"""
...
t63 = _load_module(os.path.join(_THIS_DIR, 'train.py'), 'iter63_own_train')
...
def main():
    print("=== GBM (rate_only: decay_tab_rate_3, linear_tree=True, lr=0.10, iter63) ===", flush=True)
    gbm_model, gbm_va, gbm_te, cache = t63.run(DATA_DIR, 'rate_only', seed=0, verbose=False)
    dfs, y, u = cache
    ...
    print(f"\n  [compare] iter55 blend (lr=0.10, alpha=0.10): valid=0.67451 test=0.65832")
    print(f"  [compare] iter63 blend (rate_only, alpha={alpha:.2f}): valid={best['valid']:.5f} test={te_m['primary']:.5f}")
