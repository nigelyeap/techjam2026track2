...
iter3: same as iter2_bpr_uniform, EXCEPT the per-step user sampling is now
weighted by each eligible user's positive-row count in train (pos_len),
instead of uniform over eligible users. Motivation (see LEDGER.md iter2):
uniform-per-user sampling gives every eligible user equal weight regardless
of activity, unlike pointwise SGD which implicitly sees active users more
often (it just iterates over all rows). This tests whether restoring that
activity weighting recovers the BPR loss's ranking-metric alignment without
losing to the plain FM pointwise baseline.

Motivation for BPR itself: the FM baseline trains on pointwise logloss (per-row
"is this a long_view?") but is graded on within-user ranking metrics
(GAUC / nDCG@5). BPR closes that mismatch: for each training step, sample
(user, positive row, negative row) triples from the SAME user and push
score(pos) above score(neg). Loss = -log(sigmoid(score_pos - score_neg)).

Reuses baseline.FM unmodified (same V/W/b, same Adam optimizer, same
logits()) so this is an apples-to-apples comparison against the FM
baseline: only the loss/batch-construction changes.
"""
...
def sample_pairs(rng, n_users, bs, pos_flat, pos_start, pos_len, neg_flat, neg_start, neg_len,
    ...
    users, weighted by activity (positive-row count). If given, users are picked
...
def run_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, seed=0, verbose=True,
            steps_mult=1):
    ...
    # iter3: sample users proportional to their positive-row count (activity),
    # instead of uniform. Precompute a cumulative-weight table once so the
    # per-step pick is a vectorized cumsum + searchsorted lookup.
...
if __name__ == '__main__':
    ...
    res = run_bpr(splits, k=a.k, lr=a.lr, epochs=a.epochs, bs=a.bs,
