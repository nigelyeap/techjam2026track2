"""Independent metadata tests and validation-only composition for XGBoost."""
...
def columns_for(groups):
    return common.XGB_REFERENCE_COLUMNS + [
...
def main():
    ...
    reference = common.record(
        "X0_reference", [], common.XGB_REFERENCE_COLUMNS, "xgb",
    ...
    for index, group in enumerate(common.FEATURE_GROUPS, start=1):
        ...
        model, scores, metrics, gain = common.fit_xgb(
            frames, y, users, columns, 0
        )
        row = common.record(
            f"X{index}_{group}", [group], columns, "xgb", model, metrics, gain
        )
    ...
    selected_columns = common.XGB_REFERENCE_COLUMNS
    ...
    if survivors:
        ...
        while remaining:
            ...
            for group in remaining:
                ...
                row = common.record(
                    f"XC_{'_'.join(groups)}", groups, columns, "xgb",
                    model, metrics, gain
                )
                row["delta_vs_reference"] = float(
                    metrics["primary"] - ref_metrics["primary"]
    ...
    if selected_delta >= common.PROMOTION_DELTA:
        print(f"=== confirming selected XGBoost groups {selected_groups} ===", flush=True)
        confirmation = common.confirm_representation(
            "xgb", frames, y, users, common.XGB_REFERENCE_COLUMNS,
            selected_columns, ref_metrics, selected_metrics
        )

    selected_record = common.record(
        "X_selected", selected_groups, selected_columns, "xgb",
    ...
    results = {
        "experiment": "iterYIXI10_xgboost_intrinsic_metadata",
