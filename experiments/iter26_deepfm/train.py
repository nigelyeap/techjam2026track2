"""iter26: DeepFM-style model (baseline.FM's linear+pairwise terms unchanged,
plus a hand-rolled MLP deep part -- see model.py) trained with the SAME
activity-weighted BPR pairwise loss / sampling / optimizer hyperparameters as
iter19 (itself a line-for-line copy of iter3/iter9/iter16/iter18's train.py).
Feature set is iter19's exact fused feature set, unmodified -- this iteration
only changes the model architecture axis.

build_pos_neg_index / sample_pairs are copied verbatim from iter19's train.py
(byte-identical). bpr_step is replaced by deepfm_bpr_step, which does exactly
what iter19's bpr_step does for the FM part (V/W/b), plus an extra Adam step
for the MLP's own parameters (see model.DeepFM.mlp_adam_step).
"""
...
def build_pos_neg_index(y, users):
    """Identical to iter19/iter18/iter16/iter9/iter3's train.py -- copied verbatim."""
...
def sample_pairs(rng, n_users, bs, pos_flat, pos_start, pos_len, neg_flat, neg_start, neg_len,
    ...
    """Identical to iter19's train.py -- copied verbatim."""
...
def deepfm_bpr_step(m, Xpos, Xneg):
    """DeepFM analogue of iter19's bpr_step. The FM-part gradient block
    (gV/gW/b update) is byte-for-byte the same code as iter19's bpr_step --
    it consumes the same S/E/g quantities computed the same way. The only
    addition is the deep part's own forward/backward/Adam step, whose
    ...
    # ---- FM-part gradient: identical formula/code to baseline.FM / iter19 ----
...
if __name__ == '__main__':
    ...
    print(f"\n=== iter26 deepfm hidden={hidden} use_deep={not a.no_deep} (seed={a.seed}) ===")
