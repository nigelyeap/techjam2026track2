"""iter69: decayed per-CONTENT-TAG rate feature, applying iter63's proven
mechanism (decayed rate beats decayed raw count -- see iter63) to a new
grouping key: video content tag (from video_features_basic_pure.csv,
never joined into the causal feature pipeline before iter68's static-join
experiment). iter68 found that using tag as a raw high-cardinality (44
distinct values) categorical SPLIT feature badly hurts the num_leaves=2
booster (valid 0.67168 -> 0.64268, isolated and 5-seed-confirmed). This
file tests whether the same content signal, reshaped as a smooth
Laplace-smoothed DECAYED RATE (exactly like decay_tab_rate_3, which beat
decay_tab_3's raw count in iter63), avoids that failure mode.

Byte-for-byte copy of iter63_decay_tab_rate/data_ext.py EXCEPT:
  - `_load_raw_time` now also joins each row's PRIMARY content tag (first
    of the comma-separated tag list in video_features_basic_pure.csv,
    'UNK' if missing) via video_id, appended as a new row field.
  - `compute_decay_tag_features`: new function, structurally identical to
    `compute_decay_tab_features` (same lazy-decay-per-key mechanism) but
    keyed by (user_id, tag) instead of (user_id, tab).
  - `load_ext` additionally appends decayed_tag_pos/decayed_tag_total
    columns after the existing decayed_tab_pos/decayed_tab_total block.
"""
...
TAG_HALFLIVES = [3, 7]  # days -- mirrors TAB_HALFLIVES exactly
...
_iter18_de = _load_module('iter69_iter18_data_ext', '../iter18_momentum/data_ext.py')
...
def _compute_decay_key_features(rows, key_fn, halflives):
    """Generic version of compute_decay_tab_features: same lazy-decay
    mechanism, keyed by an arbitrary key_fn(row) instead of hardcoded
    (user, tab). Used for both tab (key_fn picks r[1],r[4]) and tag
    (key_fn picks r[1],r[10]) so the two share one verified implementation
    rather than two independently-typed copies."""
    n = len(rows)
    H = len(halflives)
    order = sorted(range(n), key=lambda i: rows[i][0])
    day_mult = [0.5 ** (1.0 / h) for h in halflives]
...
def compute_decay_tag_features(rows, halflives=TAG_HALFLIVES):
    return _compute_decay_key_features(rows, lambda r: (r[1], r[10]), halflives)
...
def _load_raw_time(data_dir):
    ...
    with open(os.path.join(data_dir, 'video_features_basic_pure.csv')) as fh:
        for r in csv.DictReader(fh):
            ...
            tag = r['tag']
            vid2tag[r['video_id']] = tag.split(',')[0] if tag else 'UNK'
...
IDX = dict(date=0, user_id=1, video_id=2, author_id=3, tab=4, duration_ms=5, label=6,
           hourmin=7, time_ms=8, orig_idx=9, tag=10,
           activity=11, tab_pos=12, prior_pos=13, prior_total=14,
           last1=15, lastk_sum=16, lastk_cnt=17, gap_ms=18)
DECAY_BASE = 19
...
def _tag_halflife_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, tag_halflives=TAG_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + 2 * len(tab_halflives)
    return base + tag_halflives.index(h)


def _tag_halflife_total_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, tag_halflives=TAG_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + 2 * len(tab_halflives) + len(tag_halflives)
    return base + tag_halflives.index(h)
...
def _cache_path(halflives, tab_halflives, tag_halflives):
    key = ('-'.join(str(h) for h in halflives) + '__tab_' + '-'.join(str(h) for h in tab_halflives)
           + '__tag_' + '-'.join(str(h) for h in tag_halflives))
...
def load_ext(data_dir, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES,
             tag_halflives=TAG_HALFLIVES, K=K_DEFAULT, use_cache=True):
    cpath = _cache_path(halflives, tab_halflives, tag_halflives)
    ...
    decayed_tag_pos, decayed_tag_total = compute_decay_tag_features(flat, tag_halflives)
    ...
    for i, (r, name) in enumerate(zip(flat, owner)):
        ...
        for h in range(len(tag_halflives)):
            extra.append(decayed_tag_pos[i, h])
        for h in range(len(tag_halflives)):
            extra.append(decayed_tag_total[i, h])
...
if __name__ == '__main__':
    ...
    print("\n=== causal spot-check: decayed_tag_total (brute force) ===")
    rng = np.random.default_rng(0)
    sample_idx = rng.choice(n, size=30, replace=False)
    max_err = 0.0
    for idx in sample_idx:
        r = flat[idx]
        date, uid, tag = r[0], r[1], r[10]
        d_ord = _date_to_ordinal(date)
        earlier_same_tag = [rr for rr in flat if rr[1] == uid and rr[10] == tag and rr[0] < date]
        for h in TAG_HALFLIVES:
            tcol = _tag_halflife_total_col(h)
            manual = sum(0.5 ** ((d_ord - _date_to_ordinal(rr[0])) / h) for rr in earlier_same_tag)
            err = abs(manual - r[tcol])
            max_err = max(max_err, err)
            assert err < 1e-6, f"CAUSALITY BUG: decayed_tag_total mismatch h={h} idx={idx} manual={manual} got={r[tcol]}"
    print(f"30 random rows x {len(TAG_HALFLIVES)} tag-halflives: decayed_tag_total matches brute force "
          f"(max abs err {max_err:.2e}). No leakage detected.")

    bad = 0
    for h in TAG_HALFLIVES:
        tot_col = _tag_halflife_total_col(h)
        pos_col = _tag_halflife_col(h)
        for r in flat[:5000]:
            if r[tot_col] + 1e-9 < r[pos_col]:
                bad += 1
    assert bad == 0, f"CAUSALITY BUG: found {bad} rows where decayed_tag_total < decayed_tag_pos!"
    print(f"sanity (5000 rows x {len(TAG_HALFLIVES)} halflives): decayed_tag_total >= decayed_tag_pos everywhere. OK.")
