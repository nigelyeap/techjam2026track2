"""iter51 (part 2): blend the linear_tree=True GBM with the FM ensemble.

iter51's standalone finding: linear_tree=True at num_leaves=2 beats
iter44's constant-leaf GBM by +0.0079 valid, confirmed tight across 5
seeds (0.66915-0.66943). This checks whether it also blends better with
the FM ensemble than iter44's original GBM did (alpha=0.1, valid 0.66473)
-- reusing the exact FM-training and alpha-sweep pattern from
iter44/iter47.
"""
...
_gbm51 = _load_module(os.path.join(_THIS_DIR, 'train.py'), 'iter51_train')
...
def main():
    print("=== GBM (linear_tree=True, iter51) ===", flush=True)
    gbm_model, gbm_va, gbm_te, cache = _gbm51.run(DATA_DIR, linear_tree=True, seed=0, verbose=False)
    dfs, y, u = cache
    ...
    print("\n=== FM 5-seed ensemble (iter38 exact config) ===", flush=True)
    ...
    print(f"\n  [compare] iter44 blend (old GBM, alpha=0.1): valid=0.66473 test=0.65197")
    print(f"  [compare] iter51 blend (linear_tree GBM, alpha={alpha:.2f}): valid={best['valid']:.5f} test={te_m['primary']:.5f}")
    ...
    with open(os.path.join(_THIS_MOD_DIR, 'blend_results.json'), 'w') as f:
        json.dump({
            ...
            'iter44_blend_for_comparison': {'alpha': 0.1, 'valid': 0.66473, 'test': 0.65197},
        }, f, indent=2)
