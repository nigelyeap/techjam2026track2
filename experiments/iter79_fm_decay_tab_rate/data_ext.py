...
def compute_decay_tab_features(rows, halflives=TAB_HALFLIVES):
    """Extended from iter20's compute_decay_tab_features (byte-for-byte copy
    of iter63_decay_tab_rate/data_ext.py's version) to additionally track
    decayed_tab_total -- the same lazy-decay mechanism as decayed_tab_pos,
    but counting ALL rows in the (user, tab) key, not just positive-labeled
    ones. Together these let callers compute a Laplace-smoothed decayed
    RATE (pos+a)/(total+2a) instead of only the raw decayed positive count.
    ...
    leakage. Returns (decayed_tab_pos, decayed_tab_total), each (n, H)
    float64 arrays, column order matching `halflives`."""
    n = len(rows)
    H = len(halflives)
    order = sorted(range(n), key=lambda i: rows[i][0])
    day_mult = [0.5 ** (1.0 / h) for h in halflives]
    ...
    while i < n:
        ...
        for idx in day_idx:
            ...
            if last is not None:
                ...
                for h in range(H):
                    decayed_tab_pos[idx, h] = pstate[h] * (day_mult[h] ** gap)
                    decayed_tab_total[idx, h] = tstate[h] * (day_mult[h] ** gap)
        ...
        for idx in day_idx:
            ...
            if label == 1:
                day_pos_count[key] += 1
        for key, tc in day_total_count.items():
            last = key_last_ord.get(key)
            if last is not None:
                gap = d_ord - last
                pstate = key_pos_state[key]
...
def load_ext(data_dir, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, K=K_DEFAULT, use_cache=True):
    ...
    for i, (r, name) in enumerate(zip(flat, owner)):
        ...
        for h in range(len(tab_halflives)):
            extra.append(decayed_tab_total[i, h])  # NEW: total counts, appended after all pos columns
...
def encode_ext(splits, feature_set=('decay_rate_3', 'decay_act_3', 'tab', 'last1', 'lastk_rate', 'gap'),
    ...
    def parse_feat(name):
        ...
        if name.startswith('decay_tab_rate_'):
            return 'decay_tab_rate', float(name.rsplit('_', 1)[1])
    ...
    for name in feature_set:
        ...
        if kind == 'activity':
            ...
            extra_edges[name] = _bucket_edges([x[tcol] for x in tr], n=n_buckets)
        elif kind == 'decay_tab_rate':
            pcol = _tab_halflife_col(h, halflives, tab_halflives)
            totcol = _tab_halflife_total_col(h, halflives, tab_halflives)
            extra_edges[name] = _bucket_edges(
                [(x[pcol] + alpha) / (x[totcol] + 2 * alpha) for x in tr], n=n_buckets)
    ...
    def extra_val(x, name):
        ...
        if kind == 'activity':
            ...
            pcol = _tab_halflife_col(h, halflives, tab_halflives)
            totcol = _tab_halflife_total_col(h, halflives, tab_halflives)
            r = (x[pcol] + alpha) / (x[totcol] + 2 * alpha)
            return str(int(np.searchsorted(extra_edges[name], r)))
        elif kind == 'last1':
            v = x[IDX['last1']]
