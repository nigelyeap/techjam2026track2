...
Same reference numbers as iterXUXIA1's check (this project's current best,
HEAD at iter63 -- see iterXUXIA1/RESULT.md's baseline-version note):
...
if __name__ == '__main__':
    ...
    gva = evaluate(gbm['uva'], gbm['yva'], gbm['gbm_va_raw'])
    gte = evaluate(gbm['ute'], gbm['yte'], gbm['gbm_te_raw'])
    print(f"GBM standalone: valid={gva['primary']:.5f} test={gte['primary']:.5f}")

    fm = get_fm_ensemble(verbose=True)
    assert np.array_equal(fm['yva'], gbm['yva']) and np.array_equal(fm['yte'], gbm['yte'])
