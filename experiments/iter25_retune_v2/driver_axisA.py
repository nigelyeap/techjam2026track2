"""iter25 Axis A: Laplace-smoothing alpha resweep on iter19's exact winning
feature set (decay_rate_3 + decay_act_3 + tab + last1 + lastk_rate + gap),
3 seeds each, all other hyperparams at iter19 defaults (k=16, n_buckets=10,
lr=0.001, bs=8192, K=5).

This redoes iter21's Axis A (which was tuned against iter16's OLDER feature
set, no momentum fields) against the current actual best feature set, per
the dispatch instructions. Grid favors the lower end per iter21's finding
(alpha=0.5 beat 1.0 there).
...
(each run takes ~90-100s; whole axis ~25 min).
"""
