"""iter18: FM + activity-weighted BPR pairwise loss (identical loss/sampling
mechanism to iter9/iter3), fed the EXTENDED feature encoding from this dir's
data_ext.py, which stacks iter9's date-grouped causal history features
(activity/tab/rate) with new time_ms-level session-momentum features
(last1/lastk_rate/gap). Isolates exactly one variable vs iter9: which extra
causal fields are appended to the base 5. Loss, optimizer, sampling,
hyperparams are all a line-for-line copy of experiments/iter9_history_dense/train.py
(itself copied from iter3_bpr_weighted/train.py).
...
def run_bpr_ext(data_dir, feature_set=('activity', 'tab', 'rate'), k=16, lr=0.001, epochs=40,
                 bs=8192, patience=4, seed=0, verbose=True, steps_mult=1, K=5,
                 _cache=None):
    """_cache: optional dict to memoize load_ext()'s splits across calls in the
    same process (loading + causal-feature computation is the expensive part,
    ~4s; re-running it per seed/config would be wasteful in a sweep driver)."""
    if _cache is not None and 'splits' in _cache:
        splits = _cache['splits']
    else:
        splits = load_ext(data_dir, K=K)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--features', default='activity,tab,rate',
                    help="comma-separated subset/order of "
                         "{activity,tab,rate,last1,lastk_rate,gap}")
    ...
    print(f"\n=== iter18 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
