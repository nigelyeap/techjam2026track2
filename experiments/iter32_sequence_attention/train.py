"""iter32: FM + activity-weighted BPR pairwise loss (identical loss/sampling
mechanism to iter3/iter9/iter16/iter18/iter19/iter24), fed the extended
feature encoding from this dir's data_ext.py, which stacks iter24's exact
standing-best feature set with a new DIN/SIM-style target-attention feature
family (attn_rate_W / attn_decay_rate_H).

Loss, optimizer, sampling, hyperparams are all a line-for-line copy of
iter24's train.py (itself copied from iter3/iter9/iter19/iter20). Reuses
...
from data_ext import (load_ext, encode_ext, BASE_FIELDS, HALFLIVES, TAB_HALFLIVES,  # noqa: E402
                       WINDOWS, ATTN_DECAY_HALFLIVES, K_EMB, EMB_EPOCHS)
...
def run_bpr_ext(data_dir, feature_set=('decay_rate_2.5', 'decay_act_2.5', 'decay_tab_3',
    ...
    the (expensive, 5-traversal) feature pass across many sweep configs."""
    splits = splits_cache if splits_cache is not None else \
        load_ext(data_dir, halflives=halflives, tab_halflives=tab_halflives, K=K,
                  windows=windows, decay_halflives=decay_halflives, k_emb=k_emb, emb_epochs=emb_epochs)
    enc, dim = encode_ext(splits, feature_set=feature_set, halflives=halflives, tab_halflives=tab_halflives,
                           windows=windows, decay_halflives=decay_halflives)
...
if __name__ == '__main__':
    ...
    print(f"\n=== iter32 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
