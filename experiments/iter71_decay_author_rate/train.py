"""iter71: decayed per-author RATE as a GBM-native feature, added on top of
iter63's rate_only feature set. Same Laplace-smoothed-rate mechanism as
iter63's decay_tab_rate_3, applied to the (user, author_id) key instead of
(user, tab) -- see this dir's data_ext.py docstring for motivation.

Variants, single seed=0 first (harness-fidelity check against iter63's
exact rate_only baseline):
  - baseline:         iter63's rate_only feature set, unchanged
  - +author_rate_h3:  baseline + decay_author_rate (halflife=3d)
  - +author_rate_h7:  baseline + decay_author_rate (halflife=7d)
  - +author_rate_both: baseline + both halflives
"""
...
_de = _load_module(os.path.join(_THIS_DIR, 'data_ext.py'), 'iter71_data_ext')
ALPHA = 0.5
...
def _row_to_dict(x):
    ...
    for h in _de.AUTHOR_HALFLIVES:
        acol_pos = _de._author_halflife_col(h, _de.HALFLIVES, _de.TAB_HALFLIVES, _de.AUTHOR_HALFLIVES)
        acol_tot = _de._author_halflife_total_col(h, _de.HALFLIVES, _de.TAB_HALFLIVES, _de.AUTHOR_HALFLIVES)
        a_pos = x[acol_pos]
        a_tot = x[acol_tot]
        d[f'decay_author_rate_{h}'] = float((a_pos + ALPHA) / (a_tot + 2 * ALPHA))
    return d


VARIANT_NUM_COLS = {
    'baseline': BASE_NUM_COLS,
    'author_rate_h3': BASE_NUM_COLS + ['decay_author_rate_3'],
    'author_rate_h7': BASE_NUM_COLS + ['decay_author_rate_7'],
    'author_rate_both': BASE_NUM_COLS + ['decay_author_rate_3', 'decay_author_rate_7'],
}
...
if __name__ == '__main__':
    ...
    for tag in ('author_rate_h3', 'author_rate_h7', 'author_rate_both'):
    ...
    print("\n=== summary (seed 0) ===")
    print(f"{'variant':<20} {'valid':>9} {'test':>9} {'Δvalid':>9} {'Δtest':>9}")
    bva, bte = results['baseline']
    for tag in ['baseline', 'author_rate_h3', 'author_rate_h7', 'author_rate_both']:
        v, tt = results[tag]
        print(f"{tag:<20} {v:>9.5f} {tt:>9.5f} {v-bva:>+9.5f} {tt-bte:>+9.5f}")
