"""iter22: FM + BPR pairwise loss, fed iter16's EXACT winning feature set
(decay_rate_3, decay_act_3, tab) as model INPUT -- unchanged from iter16.

The only change vs iter16's train.py is the user-SAMPLING weight used during
BPR training: iter16 (and iter3/iter9 before it) weight per-user sampling
probability by `pos_len[user] ** alpha` (flat, undecayed positive-row count).
This iteration asks whether re-deriving that sampling weight from a
recency-decayed positive count (`decayed_pos_total`, same halflife=3d
mechanism as the decay_act_3/decay_rate_3 INPUT features, but evaluated once
as a final scalar per user, not causally per training row) changes training
dynamics for the better, now that the model's inputs already encode recency.
See data_ext.compute_final_decayed_pos for the exact formula/leakage
reasoning (it's a training-time-only weight, never enters the feature
vector). Everything else (loss, optimizer, hyperparams) is an unmodified copy
of iter16/iter9/iter3's train.py.
"""
...
def run_bpr_ext(data_dir, feature_set=('rate',), halflives=HALFLIVES, k=16, lr=0.001, epochs=40,
                 bs=8192, patience=4, seed=0, verbose=True, steps_mult=1, splits_cache=None,
                 sampling_mode='flat', alpha=1.0, decay_halflife=3):
    """splits_cache: optional pre-loaded load_ext() result, to avoid recomputing
    the (expensive) feature pass across many sweep configs.

    sampling_mode: 'flat' = iter16/iter9/iter3's original scheme, per-user BPR
      sampling weight = pos_len[user] ** alpha (raw undecayed positive-row
      count). 'decay' (iter22) = weight = decayed_pos_total[user] ** alpha,
      where decayed_pos_total is the recency-decayed positive count (same
      halflife mechanism as the decay_act_3/decay_rate_3 INPUT features,
      evaluated once as a final per-user scalar -- see
      data_ext.compute_final_decayed_pos). Only affects which users get
      sampled more often as BPR anchors during training; does not touch the
      feature vectors, which are identical between modes.
    """
    splits = splits_cache if splits_cache is not None else load_ext(data_dir, halflives=halflives)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--features', default='rate',
                    help="comma-separated subset/order of "
                         "{activity,tab,rate,decay_rate_H,decay_act_H} where H in "
                         "{1,3,7,14,30}, e.g. 'decay_rate_7' or 'decay_rate_7,tab' or "
                         "'rate,decay_rate_7'")
    ...
    ap.add_argument('--sampling_mode', default='flat', choices=['flat', 'decay'],
                    help="'flat' = pos_len**alpha (iter16/iter9/iter3 original); "
    ...
    res = run_bpr_ext(a.data_dir, feature_set=feature_set, k=a.k, lr=a.lr, epochs=a.epochs,
                       ...
                       steps_mult=a.steps_mult, sampling_mode=a.sampling_mode, alpha=a.alpha,
                       decay_halflife=a.decay_halflife)
    print(f"\n=== iter22 fm+bpr+{'+'.join(feature_set)} "
