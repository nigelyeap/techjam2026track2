"""iter20: FM + activity-weighted BPR pairwise loss (identical loss/sampling
mechanism to iter3/iter9/iter16), fed the EXTENDED feature encoding from this
directory's data_ext.py which supports iter9's flat causal features
(activity/tab/rate), iter16-style recency-decayed rate/activity
(decay_rate_H/decay_act_H) over a finer half-life grid, AND a NEW decayed
tab_pos feature (decay_tab_H).

Loss, optimizer, sampling, hyperparams are all a line-for-line copy of
experiments/iter16_recency_decay/train.py (itself copied from iter9/iter3).
...
from baseline import FM, sigmoid        # noqa: E402  same FM class as iter3/iter9/iter16

from data_ext import load_ext, encode_ext, BASE_FIELDS, HALFLIVES, TAB_HALFLIVES  # noqa: E402


def build_pos_neg_index(y, users):
    """Identical to iter3/iter9/iter16's train.py — copied verbatim."""
...
def run_bpr_ext(data_dir, feature_set=('rate',), halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES,
                 k=16, lr=0.001, epochs=40, bs=8192, patience=4, seed=0, verbose=True, steps_mult=1,
                 splits_cache=None):
...
if __name__ == '__main__':
    ...
    ap.add_argument('--features', default='rate',
                    help="comma-separated subset/order of "
                         "{activity,tab,rate,decay_rate_H,decay_act_H,decay_tab_H} where H in "
                         "HALFLIVES (rate/act) or TAB_HALFLIVES (tab), e.g. 'decay_rate_3' or "
                         "'decay_rate_3,decay_act_3,tab' or 'decay_rate_3,decay_act_3,decay_tab_7'")
    ...
    print(f"\n=== iter20 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
