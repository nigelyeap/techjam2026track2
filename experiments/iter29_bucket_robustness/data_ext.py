"""iter29: SHIFTED-SPLIT variant of iter25's data_ext.py, for a train/valid
/test date-boundary robustness check on iter25's n_buckets=20 finding. Only
change from iter25's data_ext.py: `_load_raw_time` uses a locally-defined
SPLITS_SHIFTED dict (below) instead of importing SPLITS from ../../data.py,
so this module never touches data.py and produces a genuinely different
train/valid/test partition from real log data. Everything else (feature
computation, encode_ext, alpha/n_buckets plumbing) is untouched from iter25.

Why a 3-day earlier shift, not more: the raw log's actual date coverage is
20220409-20220508 (verified by direct inspection of both log_standard CSVs --
NOT 20220408 as the nominal SPLITS lower bound in data.py suggests; 04-08 has
zero rows). The official split (train 04-08..21 / valid 04-22..28 / test
04-29..05-08) already consumes the ENTIRE available date range end-to-end
(13 effective train days + 7 valid + 10 test = 30 days = the full log), so
there is ZERO slack to shift the window while preserving all three window
widths -- train's start is already pinned to the very first date with data.
A shift therefore has to shrink train's width (it cannot extend earlier than
20220409) and drop the same number of days off the tail (past 20220508,
which does not exist either). Row-count-by-date inspection showed per-day
volume decays sharply from ~279k rows/day (04-11) down to a ~14-20k/day
plateau by ~04-17 onward and stays roughly flat through 05-08 -- so a small
shift barely changes split sizes (see table below), while a larger shift
(e.g. 5+ days) starts noticeably shrinking train. We chose delta=3 days as
the smallest shift that still produces day-for-day DIFFERENT valid/test
boundaries never officially evaluated as an exact window before, while
keeping per-split row counts close to the original (train 1,079,797 vs
1,141,112 original; valid 143,394 vs 124,909; test 170,150 vs 170,588 --
see RESULT.md for the full derivation). This is intentionally a modest,
honestly-reported shift per the dispatch instructions, not an attempt at a
maximally-different window.

--- original module docstring below (iter25, describing the shared feature
logic, which this file does not modify) ---
...
# iter29: deliberately does NOT import SPLITS from ../../data.py -- this
# module needs a DIFFERENT train/valid/test date partition (shifted 3 days
# earlier) for the robustness check. data.py is never touched.
SPLITS_SHIFTED = {'train': (20220405, 20220418),
                   'valid': (20220419, 20220425),
                   'test':  (20220426, 20220505)}
SPLITS = SPLITS_SHIFTED  # local name kept so the rest of this file (copied from iter25) needs no other edits
...
def encode_ext(splits, feature_set=('decay_rate_3', 'decay_act_3', 'tab'), halflives=HALFLIVES,
    ...
    `alpha`: Laplace-smoothing constant used in the rate/decay_rate/lastk_rate
    formulas (iter25: made sweepable, mirroring iter21's change to iter16's
    data_ext.py -- module-level ALPHA=1.0 remains the default).
    `n_buckets`: quantile bucket count for ALL bucketed continuous features,
    including dur_bucket (iter25: made sweepable, same as iter21).
    ...
    for name in feature_set:
        ...
        if kind == 'activity':
            ...
            extra_edges[name] = _bucket_edges([(x[pcol] + alpha) / (x[tcol] + 2 * alpha) for x in tr], n=n_buckets)
        elif kind == 'decay_act':
            pcol, tcol = _halflife_col(h, halflives)
