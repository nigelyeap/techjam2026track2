"""iter42: adds ONE new causal feature axis on top of iter27/iter38's proven
feature set: a decayed rate of "other engagement" (is_like OR is_follow OR
is_comment OR is_forward on that row), computed with the exact same
lazy-decay mechanism as the existing decay_rate/decay_act features but keyed
off a different raw signal than the long_view label. Motivation: these are
real behavioral signals never used anywhere in this project's feature set so
far (every prior iteration only ever used long_view/click history), and they
are NOT derivable from duration/play-time (no leakage risk -- like/follow/
comment/forward are independent user actions, not a play-time-ratio
threshold like long_view is). Only the historical (decayed, past-only) rate
is used as an input feature; the same-row values are never read.

Everything below this point is otherwise copied verbatim from iter27's
data_ext.py; search for "iter42" to find the additions.
...
# iter42's new axis: decayed "other engagement" (like/follow/comment/forward)
# rate. Fixed at the already-established winning halflife (2.5d, iter24) to
# keep this experiment tractable -- only sweep it separately if the feature
# itself proves useful.
ENGAGE_HALFLIVES = [2.5]  # days
...
def compute_decay_features(rows, halflives=HALFLIVES, label_col=6):
    ...
    float64 array.
    iter42 addition: `label_col` (default 6 = the long_view label, as
    before) lets this same causal mechanism be reused for a different
    binary "positive" signal -- e.g. the `engaged` column (10) -- while
    `decayed_total` (per-user activity decay, independent of which column
    counts as positive) stays correct either way."""
    n = len(rows)
    H = len(halflives)
    order = sorted(range(n), key=lambda i: rows[i][0])
...
def _load_raw_time(data_dir):
    ...
    for f in ('log_standard_4_08_to_4_21_pure.csv', 'log_standard_4_22_to_5_08_pure.csv'):
        with open(os.path.join(data_dir, f)) as fh:
            for r in csv.DictReader(fh):
                engaged = 1 if (r['is_like'] != '0' or r['is_follow'] != '0'
                                 or r['is_comment'] != '0' or r['is_forward'] != '0') else 0
                rows.append((int(r['date']), r['user_id'], r['video_id'],
                             vid2author.get(r['video_id'], 'UNK'), r['tab'],
                             float(r['duration_ms']), 1 if r[LABEL] != '0' else 0,
                             int(r['hourmin']), int(r['time_ms']), len(rows), engaged))
...
def _engage_halflife_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, engage_halflives=ENGAGE_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + len(tab_halflives)
    pos = engage_halflives.index(h)
    return base + 2 * pos, base + 2 * pos + 1
...
def load_ext(data_dir, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, K=K_DEFAULT, use_cache=True):
    ...
    # iter42: fifth independent causal traversal, same mechanism as #3 above
    # but keyed off `engaged` (col 10: like/follow/comment/forward) instead
    # of the long_view label -- a genuinely new behavioral signal.
    decayed_engage_pos, decayed_engage_total = compute_decay_features(
        flat, ENGAGE_HALFLIVES, label_col=IDX['engaged'])
    ...
    for i, (r, name) in enumerate(zip(flat, owner)):
        ...
        for h in range(len(ENGAGE_HALFLIVES)):
            extra.append(decayed_engage_pos[i, h])
            extra.append(decayed_engage_total[i, h])
...
def encode_ext(splits, feature_set=('decay_rate_3', 'decay_act_3', 'tab', 'last1', 'lastk_rate', 'gap'),
    ...
    def parse_feat(name):
        ...
        if name.startswith('decay_engage_'):
            return 'decay_engage', float(name.rsplit('_', 1)[1])
    ...
    for name in feature_set:
        ...
        if kind == 'activity':
            ...
            extra_edges[name] = _bucket_edges([x[tcol] for x in tr], n=n_buckets)
        elif kind == 'decay_engage':
            pcol, tcol = _engage_halflife_col(h)
            extra_edges[name] = _bucket_edges(
                [(x[pcol] + alpha) / (x[tcol] + 2 * alpha) for x in tr], n=n_buckets)
