"""iter73: decayed per-(user, hour-of-day-bucket) RATE -- same lazy-decay-per-
key mechanism as iter63's decay_tab_rate_3, applied to a time-of-day bucket
instead of `tab`.

Motivation (per iter72's revised diagnosis, correcting iter71's falsified
"coarseness" theory): the decayed-rate transformation adds value specifically
on axes that are CONTEXT/session properties (like `tab` -- which recommender
surface served the video), not intrinsic CONTENT properties -- iter69 (tag),
iter71 (author), and iter72 (duration bucket, deliberately built as coarse as
tab) all REJECTed cleanly despite iter72 matching tab's cardinality, which
falsifies coarseness as the deciding factor. `hourmin` (time-of-day the
impression was served) is a genuinely different kind of signal: it describes
the *circumstance of the visit*, not a property of the video being shown --
structurally closer to `tab` than to duration/tag/author. It has been carried
in the row tuple since iter18/iter48, but iter48 only tested a raw sin/cos
numeric encoding on the now-superseded pre-linear_tree, pre-decay_tab_rate
iter44 baseline; a *decayed rate* by hour-bucket, on the *current* rate_only
representation, has never been tried.

Bucket edges: 6 four-hour buckets over the 0-23 hour range (0-3, 4-7, 8-11,
12-15, 16-19, 20-23) -- comparable cardinality to `tab`'s ~7 levels and to
iter72's 6 duration buckets, a fixed schedule property with no label
information, so no leakage risk.

Byte-for-byte copy of iter72_decay_durbucket_rate/data_ext.py, with
compute_decay_hour_features added (identical algorithm, keyed on
(user_id, hour_bucket(hourmin)) instead of (user_id, duration_bucket)), and
load_ext extended to append the new columns after the duration-decay block.
"""
...
def _hour_bucket(hourmin):
    hour = (hourmin // 100) % 24
    return hour // 4  # 6 buckets: 0-3, 4-7, 8-11, 12-15, 16-19, 20-23
...
_iter18_de = _load_module('iter73_iter18_data_ext', '../iter18_momentum/data_ext.py')
...
def compute_decay_hour_features(rows, halflives=HOUR_HALFLIVES):
    return _compute_decay_key_features(rows, lambda r: (r[1], _hour_bucket(r[7])), halflives)
...
def _hour_halflife_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, hour_halflives=HOUR_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + 2 * len(tab_halflives)
    return base + hour_halflives.index(h)


def _hour_halflife_total_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, hour_halflives=HOUR_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + 2 * len(tab_halflives) + len(hour_halflives)
    return base + hour_halflives.index(h)
...
def _cache_path(halflives, tab_halflives, hour_halflives):
    key = ('-'.join(str(h) for h in halflives) + '__tab_' + '-'.join(str(h) for h in tab_halflives)
           + '__hour_' + '-'.join(str(h) for h in hour_halflives))
...
def load_ext(data_dir, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES,
             hour_halflives=HOUR_HALFLIVES, K=K_DEFAULT, use_cache=True):
    cpath = _cache_path(halflives, tab_halflives, hour_halflives)
    ...
    decayed_hour_pos, decayed_hour_total = compute_decay_hour_features(flat, hour_halflives)
    ...
    for i, (r, name) in enumerate(zip(flat, owner)):
        ...
        for h in range(len(hour_halflives)):
            extra.append(decayed_hour_pos[i, h])
        for h in range(len(hour_halflives)):
            extra.append(decayed_hour_total[i, h])
...
if __name__ == '__main__':
    ...
    print("\n=== hour-bucket distribution (train) ===")
    from collections import Counter
    c = Counter(_hour_bucket(r[7]) for r in ext['train'])
    print(dict(sorted(c.items())))

    print("\n=== causal spot-check: decayed_hour_total (brute force) ===")
    rng = np.random.default_rng(0)
    sample_idx = rng.choice(n, size=30, replace=False)
    max_err = 0.0
    for idx in sample_idx:
        r = flat[idx]
        date, uid, bucket = r[0], r[1], _hour_bucket(r[7])
        d_ord = _date_to_ordinal(date)
        earlier_same_bucket = [rr for rr in flat if rr[1] == uid and _hour_bucket(rr[7]) == bucket and rr[0] < date]
        for h in HOUR_HALFLIVES:
            tcol = _hour_halflife_total_col(h)
            manual = sum(0.5 ** ((d_ord - _date_to_ordinal(rr[0])) / h) for rr in earlier_same_bucket)
            err = abs(manual - r[tcol])
            max_err = max(max_err, err)
            assert err < 1e-6, f"CAUSALITY BUG: decayed_hour_total mismatch h={h} idx={idx} manual={manual} got={r[tcol]}"
    print(f"30 random rows x {len(HOUR_HALFLIVES)} hour-halflives: decayed_hour_total matches brute force "
          f"(max abs err {max_err:.2e}). No leakage detected.")

    bad = 0
    for h in HOUR_HALFLIVES:
        tot_col = _hour_halflife_total_col(h)
        pos_col = _hour_halflife_col(h)
        for r in flat[:5000]:
            if r[tot_col] + 1e-9 < r[pos_col]:
                bad += 1
    assert bad == 0, f"CAUSALITY BUG: found {bad} rows where decayed_hour_total < decayed_hour_pos!"
    print(f"sanity (5000 rows x {len(HOUR_HALFLIVES)} halflives): decayed_hour_total >= decayed_hour_pos everywhere. OK.")
