"""iter73: decayed per-(user, hour-of-day-bucket) RATE as a GBM-native
feature, added on top of iter63's rate_only feature set. Same mechanism as
decay_tab_rate_3, applied to a bucketed hourmin key instead of tab -- see
this dir's data_ext.py docstring for motivation (time-of-day is a CONTEXT/
session property like tab, unlike duration/tag/author which iter69/71/72
showed are all-null content properties, regardless of cardinality).
"""
...
_de = _load_module(os.path.join(_THIS_DIR, 'data_ext.py'), 'iter73_data_ext')
ALPHA = 0.5
...
def _row_to_dict(x):
    ...
    for h in _de.HOUR_HALFLIVES:
        hcol_pos = _de._hour_halflife_col(h, _de.HALFLIVES, _de.TAB_HALFLIVES, _de.HOUR_HALFLIVES)
        hcol_tot = _de._hour_halflife_total_col(h, _de.HALFLIVES, _de.TAB_HALFLIVES, _de.HOUR_HALFLIVES)
        h_pos = x[hcol_pos]
        h_tot = x[hcol_tot]
        d[f'decay_hour_rate_{h}'] = float((h_pos + ALPHA) / (h_tot + 2 * ALPHA))
    return d


VARIANT_NUM_COLS = {
    'baseline': BASE_NUM_COLS,
    'hour_rate_h3': BASE_NUM_COLS + ['decay_hour_rate_3'],
    'hour_rate_h7': BASE_NUM_COLS + ['decay_hour_rate_7'],
    'hour_rate_both': BASE_NUM_COLS + ['decay_hour_rate_3', 'decay_hour_rate_7'],
}
...
if __name__ == '__main__':
    ...
    for tag in ('hour_rate_h3', 'hour_rate_h7', 'hour_rate_both'):
    ...
    for tag in ['baseline', 'hour_rate_h3', 'hour_rate_h7', 'hour_rate_both']:
        v, tt = results[tag]
