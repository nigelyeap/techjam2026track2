"""iter12: FM + activity-weighted BPR pairwise loss (identical loss/sampling
mechanism as iter3/iter9), fed the EXTENDED feature encoding from this
directory's data_ext.py, which supports both iter9's USER-side causal
features (activity, tab, rate) and the new ITEM-side causal features
(video_pop, author_rate) in any combination.

Loss, optimizer, sampling, hyperparams are all a line-for-line copy of
experiments/iter9_history_dense/train.py (itself copied from iter3). This
isolates exactly one variable vs iter9: the feature set.
...
def build_pos_neg_index(y, users):
    """Identical to iter3/iter9 train.py — copied verbatim."""
...
def run_bpr_ext(data_dir, feature_set=(), k=16, lr=0.001, epochs=40, bs=8192,
                 patience=4, seed=0, verbose=True, steps_mult=1, preloaded=None):
    """feature_set: subset/order of {'activity','tab','rate','video_pop','author_rate'}.
    preloaded: optional (splits_dict) from load_ext(), to avoid recomputing the
    causal-feature pass across multiple seeds/configs in a driver loop."""
    splits = preloaded if preloaded is not None else load_ext(data_dir)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--features', default='activity,tab,rate',
                    help="comma-separated subset/order of "
                         "{activity,tab,rate,video_pop,author_rate}")
    ...
    print(f"\n=== iter12 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
