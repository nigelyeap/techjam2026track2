"""iter9: FM + activity-weighted BPR pairwise loss (the same loss/sampling
mechanism as the current best, iter3), fed the EXTENDED feature encoding
from data_ext.py (base 5 fields + coarse causal history feature(s)) instead
of the plain 5-field encoding from ../../data.py. This isolates exactly one
variable vs iter3: the feature set. Loss, optimizer, sampling, hyperparams
are all a line-for-line copy of experiments/iter3_bpr_weighted/train.py.
...
def run_bpr_ext(data_dir, feature_set=('activity',), k=16, lr=0.001, epochs=40, bs=8192,
                 patience=4, seed=0, verbose=True, steps_mult=1, n_buckets=10):
...
if __name__ == '__main__':
    ...
    res = run_bpr_ext(a.data_dir, feature_set=feature_set, k=a.k, lr=a.lr, epochs=a.epochs,
                       ...
                       steps_mult=a.steps_mult, n_buckets=a.n_buckets)
    print(f"\n=== iter9 fm+bpr_weighted+{'+'.join(feature_set)} (seed={a.seed}) ===")
