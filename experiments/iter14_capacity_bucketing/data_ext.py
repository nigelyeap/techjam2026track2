...
def encode_ext(splits, feature_set=('activity',), n_buckets=10):
    ...
    fields (dur_bucket via train-fit quantile edges, ALWAYS 10 buckets,
    matching iter9/data.py unchanged); each requested extra field gets its
    own train-fit quantile bucketing into `n_buckets` buckets (default 10,
    matching iter9) EXCEPT 'rate' which is Laplace-smoothed first.
    Returns (enc, field_dims_sum) with enc[name] = (X, y, users)."""
    tr = splits['train']
    dur_edges = _bucket_edges([x[5] for x in tr])  # dur_bucket stays fixed at 10 (base field, iter9-identical)
    ...
    if 'activity' in feature_set:
        extra_edges['activity'] = _bucket_edges([x[7] for x in tr], n=n_buckets)
    if 'tab' in feature_set:
        extra_edges['tab'] = _bucket_edges([x[8] for x in tr], n=n_buckets)
    if 'rate' in feature_set:
        def rate_of(x):
            return (x[9] + ALPHA) / (x[10] + 2 * ALPHA)
        extra_edges['rate'] = _bucket_edges([rate_of(x) for x in tr], n=n_buckets)
