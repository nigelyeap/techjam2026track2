"""iter31: this module is a VERBATIM COPY of iter24's data_ext.py (feature
pipeline: activity/tab_pos/rate, exponential-decay rate/act/tab, and
time_ms-level momentum last1/lastk_rate/gap -- iter24's exact winning
feature set) with ONE addition at the bottom: `load_aux_labels()`, which
loads the 5 auxiliary engagement labels (`is_click`, `is_like`,
`is_follow`, `is_comment`, `is_forward`) for the multi-task auxiliary loss
this iteration adds in train.py.

IMPORTANT — leakage-by-construction argument for the auxiliary labels:
`load_aux_labels()` is a completely separate code path from `encode_ext()`
above. The 5 auxiliary labels it returns are NEVER joined into the X
feature matrix, NEVER passed through `encode_ext`, and NEVER become model
INPUT for any row in any split. They are consumed only inside train.py's
training loop, and only for TRAIN-split rows (indexed by the same
Xpos_rows/Xneg_rows already drawn from the train-only BPR pos/neg index),
as auxiliary GRADIENT targets -- exactly analogous to how iter22/iter23's
decay-aware BPR sampling weight uses a train-only aggregate to shape
*how* training samples are drawn/weighted, never as a feature seen at
inference. valid/test entries returned by `load_aux_labels` exist only for
symmetry/inspection and must never be read by anything gradient-related.

Everything below `load_aux_labels` (the original iter24 feature pipeline
and its own causality self-tests) is unchanged. Original iter24 docstring
follows for context:

---
iter24: iter19's fused feature set (decay_rate/decay_act + flat tab +
momentum last1/lastk_rate/gap) refined with iter20's two findings, both
re-tested for the first time IN THE PRESENCE of iter19's momentum fields:

  (a) finer decay_rate_H/decay_act_H halflife grid around iter16/iter20's
      3d/2.5d peaks -- HALFLIVES = [2, 2.5, 3, 3.5] days.
  (b) decayed `tab_pos` (decay_tab_H) in place of iter19's flat `tab` --
      TAB_HALFLIVES = [3, 7] days (iter20's grid; iter20 found 3d best).

Neither refinement was previously tested together with iter18/iter19's
momentum features (last1/lastk_rate/gap) -- both were tuned purely against
iter16's OLDER feature set which lacks momentum entirely. Motivation for
re-sweeping the halflife (rather than assuming 2.5d still wins): momentum
already captures some of the very-short-horizon signal a short halflife
would otherwise be fighting for, so the joint optimum could plausibly shift
back toward 3d or even longer.

Architecture (mirrors iter19's own design choice, explicitly required by the
dispatch prompt): THREE independent causal traversals over the same flat
per-row data, joined onto the same rows by row index -- NOT merged into one
traversal. This keeps each family (decay, decayed-tab, momentum) provably
free of cross-contamination from the others (verified in __main__ PART D).

  1. compute_causal_features   -- iter9/iter16/iter20's flat date-grouped
                                   activity/tab_pos/prior_pos/prior_total.
                                   Copied verbatim from iter20's data_ext.py
                                   (iter20 itself copied it verbatim from
                                   iter9/iter16 -- same code, unmodified).
  2. compute_decay_features    -- iter16/iter20's exponential-decay
                                   rate/act. Copied verbatim from iter20's
                                   data_ext.py (fine HALFLIVES grid).
  3. compute_decay_tab_features -- iter20's NEW decayed tab_pos. Copied
                                   verbatim from iter20's data_ext.py.
  4. compute_momentum_features -- iter18's time_ms-level last1/lastk/gap.
                                   ...
                                   iter19 did -- needs user_id@1, label@6,
                                   time_ms@8, orig_idx@9, all present in this
                                   module's row tuple (iter19's 10-col
                                   loader, reused verbatim below).
...
authoritative index map):
  ...
  then decay_tab_h0, decay_tab_h1, ...                           <- decayed tab_pos (iter20 NEW)
...
def load_aux_labels(data_dir):
    """Loads the 5 auxiliary engagement labels for ALL rows, split by the
    official date ranges, in EXACTLY the same order as `_load_raw_time`'s
    per-split row lists (same two log files read in the same order, same
    date-range filter predicate `lo <= date <= hi`). Because both this
    function and `_load_raw_time` are independent filters of the SAME
    underlying file-order sequence with the SAME (disjoint) predicates, the
    filtered subsequence order is identical by construction -- so
    `load_aux_labels(...)['train'][label][i]` lines up with row i of
    `splits['train']` / `Xtr[i]` for any `Xtr` produced by `encode_ext` on
    `load_ext`'s output, with no extra join/sort step needed.

    Returns {split_name: {label_name: float32 array}}. These arrays are
    NOT features -- see the module docstring's leakage-by-construction
    argument. train.py must only ever read the 'train' entry.
    """
...
if __name__ == '__main__':
    ...
    # ================================================================
    # PART E (iter31-specific): auxiliary multi-task label alignment +
    # leakage-by-construction sanity check.
    # ================================================================
    print("\n=== PART E: auxiliary label (is_click/is_like/is_follow/is_comment/"
          "is_forward) alignment + base-rate sanity check ===")
    aux = load_aux_labels(a.data_dir)
    for name in ('train', 'valid', 'test'):
        n_split = len(ext[name])
        n_aux = len(aux[name][AUX_LABELS[0]])
        assert n_aux == n_split, (f"ALIGNMENT BUG: {name} aux label count {n_aux} != "
                                   f"feature-row count {n_split}")
        print(f"  {name:5s} rows={n_split:8d}  " +
              "  ".join(f"{c}={aux[name][c].mean()*100:5.2f}%" for c in AUX_LABELS))
    # Spot-check a handful of train rows against a fresh independent read of
    # the raw CSVs (not reusing any of the loaders above), confirming
    # load_aux_labels lines up 1:1 with the same-order train row list that
    # encode_ext turns into Xtr.
    train_rows_raw = []
    for f in ('log_standard_4_08_to_4_21_pure.csv', 'log_standard_4_22_to_5_08_pure.csv'):
        with open(os.path.join(a.data_dir, f)) as fh:
            for r in csv.DictReader(fh):
                ...
                if 20220408 <= d <= 20220421:
                    train_rows_raw.append(r)
    assert len(train_rows_raw) == len(ext['train']), "ALIGNMENT BUG: train row count mismatch"
    rng2 = np.random.default_rng(1)
    for i in rng2.choice(len(train_rows_raw), size=20, replace=False):
        r = train_rows_raw[i]
        for c in AUX_LABELS:
            expect = 1.0 if r[c] != '0' else 0.0
            got = aux['train'][c][i]
            assert expect == got, f"ALIGNMENT BUG: row {i} col {c} expected {expect} got {got}"
        # also cross-check user/video against the causal-feature row tuple to
        # confirm it's truly the same row, not just a coincidental label match
        assert ext['train'][i][IDX['user_id']] == r['user_id']
        assert ext['train'][i][IDX['video_id']] == r['video_id']
    print("  20 random train rows: aux labels + user_id/video_id all match a fresh independent "
          "CSV read at the same row index. load_aux_labels is correctly index-aligned with "
          "encode_ext's train row order -- no leakage risk: these labels are read from a wholly "
          "separate code path than encode_ext's feature pipeline and are never joined into X.")
