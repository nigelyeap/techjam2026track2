"""iter77: decayed rate keyed on the CROSS of (user, tab, duration-bucket)
-- not another instance of Round 22's now-closed "decay this new axis alone"
family, but a test of whether crossing the ONE proven axis (`tab`, iter16/
iter63) with the best-explored secondary axis (duration-bucket, iter72 --
null on its own) adds finer-grained interaction signal beyond what either
axis captures independently. iter70 already showed the leaf-linear model
cannot represent a genuine multiplicative interaction between two existing
*rate* columns without a dedicated term (and that a naive product term of
decay_rate_2.5 x decay_tab_rate_3 actively hurts) -- this is a different
question: does joint conditioning at the RAW-COUNT level (before computing
a rate) reveal signal that flat `tab` conditioning misses? E.g. "this user's
engagement rate on SHORT videos specifically in the follow feed" could differ
meaningfully from "this user's rate in the follow feed generally" if short-
vs-long preference is itself feed-dependent.

Cardinality: tab (~7 levels) x duration-bucket (6 levels) = up to ~42
per-user cells -- comparable to author-level granularity (iter71, null) but
structurally different since it's anchored on tab (the working axis) rather
than replacing it.

Byte-for-byte copy of iter72_decay_durbucket_rate/data_ext.py, with
compute_decay_tab_dur_cross_features added (keyed on
(user_id, tab, duration_bucket(duration_ms)) jointly instead of either axis
alone), and load_ext extended to append the new columns after the
duration-bucket-decay block.
"""
...
_iter18_de = _load_module('iter77_iter18_data_ext', '../iter18_momentum/data_ext.py')
...
def compute_decay_tab_dur_cross_features(rows, halflives=CROSS_HALFLIVES):
    return _compute_decay_key_features(rows, lambda r: (r[1], r[4], _dur_bucket(r[5])), halflives)
...
def _cross_halflife_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, cross_halflives=CROSS_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + 2 * len(tab_halflives)
    return base + cross_halflives.index(h)


def _cross_halflife_total_col(h, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES, cross_halflives=CROSS_HALFLIVES):
    base = DECAY_BASE + 2 * len(halflives) + 2 * len(tab_halflives) + len(cross_halflives)
    return base + cross_halflives.index(h)
...
def _cache_path(halflives, tab_halflives, cross_halflives):
    key = ('-'.join(str(h) for h in halflives) + '__tab_' + '-'.join(str(h) for h in tab_halflives)
           + '__cross_' + '-'.join(str(h) for h in cross_halflives))
...
def load_ext(data_dir, halflives=HALFLIVES, tab_halflives=TAB_HALFLIVES,
             cross_halflives=CROSS_HALFLIVES, K=K_DEFAULT, use_cache=True):
    cpath = _cache_path(halflives, tab_halflives, cross_halflives)
    ...
    decayed_cross_pos, decayed_cross_total = compute_decay_tab_dur_cross_features(flat, cross_halflives)
    ...
    for i, (r, name) in enumerate(zip(flat, owner)):
        ...
        for h in range(len(cross_halflives)):
            extra.append(decayed_cross_pos[i, h])
        for h in range(len(cross_halflives)):
            extra.append(decayed_cross_total[i, h])
...
if __name__ == '__main__':
    ...
    print("\n=== cross-key cell count (train) ===")
    from collections import Counter
    c = Counter((r[4], _dur_bucket(r[5])) for r in ext['train'])
    print(f"  {len(c)} distinct (tab, dur_bucket) cells; smallest 5: {sorted(c.items(), key=lambda kv: kv[1])[:5]}")

    print("\n=== causal spot-check: decayed_cross_total (brute force) ===")
    rng = np.random.default_rng(0)
    sample_idx = rng.choice(n, size=30, replace=False)
    max_err = 0.0
    for idx in sample_idx:
        r = flat[idx]
        date, uid, tab, bucket = r[0], r[1], r[4], _dur_bucket(r[5])
        d_ord = _date_to_ordinal(date)
        earlier_same_cell = [rr for rr in flat if rr[1] == uid and rr[4] == tab
                              and _dur_bucket(rr[5]) == bucket and rr[0] < date]
        for h in CROSS_HALFLIVES:
            tcol = _cross_halflife_total_col(h)
            manual = sum(0.5 ** ((d_ord - _date_to_ordinal(rr[0])) / h) for rr in earlier_same_cell)
            err = abs(manual - r[tcol])
            max_err = max(max_err, err)
            assert err < 1e-6, f"CAUSALITY BUG: decayed_cross_total mismatch h={h} idx={idx} manual={manual} got={r[tcol]}"
    print(f"30 random rows x {len(CROSS_HALFLIVES)} cross-halflives: decayed_cross_total matches brute force "
          f"(max abs err {max_err:.2e}). No leakage detected.")

    bad = 0
    for h in CROSS_HALFLIVES:
        tot_col = _cross_halflife_total_col(h)
        pos_col = _cross_halflife_col(h)
        for r in flat[:5000]:
            if r[tot_col] + 1e-9 < r[pos_col]:
                bad += 1
    assert bad == 0, f"CAUSALITY BUG: found {bad} rows where decayed_cross_total < decayed_cross_pos!"
    print(f"sanity (5000 rows x {len(CROSS_HALFLIVES)} halflives): decayed_cross_total >= decayed_cross_pos everywhere. OK.")
